...
func RunDuckDBMigrations(dsn string, migrationsPath string) error {
	...
	return runMigrations(db, "DuckDB", migrationsPath)
}
...
func RunPostgresMigrations(dsn string, migrationsPath string) error {
	...
	return runMigrations(db, "PostgreSQL", migrationsPath)
}

// runMigrations is the shared migration engine.
// It creates the schema_migrations table, discovers up migrations,
// and applies them in version order within transactions.
func runMigrations(db *sql.DB, label string, migrationsPath string) error {
	...
	for version := currentVersion + 1; version <= highestVersion; version++ {
		if upFile, ok := upFiles[version]; ok {
			...
			log.Printf("Applied %s migration version %d", label, version)
		}
	}

	if highestVersion > currentVersion {
		log.Printf("%s migrations complete. Applied %d migration(s)", label, highestVersion-currentVersion)
	} else {
		log.Printf("%s migrations up-to-date. No migrations applied.", label)
	}
