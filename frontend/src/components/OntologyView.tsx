...
export const OntologyView: React.FC<OntologyViewProps> = ({ ontologyRaw, setOntologyRaw, onEmerge, onSave, inline = false, isLoading, error }) => {
    ...
    <div className={(inline ? '' : 'max-w-6xl mx-auto ') + 'space-y-6 h-full flex flex-col'}>
        <div className={(inline ? '' : 'bg-surface p-6 rounded-lg border border-border shadow-sm ') + 'flex justify-between items-center'}>
           ...
           <div className="flex space-x-3">
              <button onClick={onEmerge} className="flex items-center space-x-2 bg-warning/10 text-warning px-6 py-3 rounded-lg font-bold hover:bg-warning/10 transition-all shadow-sm border border-warning/20" aria-label="Emerge automatic ontology">
                <Zap size={20} />
                <span>Emergenza Automatica</span>
              </button>
              <button onClick={onSave} className="flex items-center space-x-2 bg-primary text-background px-6 py-3 rounded-lg font-bold hover:bg-primary/90 transition-all shadow-lg " aria-label="Publish ontology model">
                <Save size={20} />
