...
// Replaces the old slashCommands.ts inline array and CopilotView.tsx inline array.
...
export const COMMAND_REGISTRY: UnifiedCommand[] = [
  ...
  { name: '/agents', description: 'Gestisci agenti', usage: '/agents', action: 'SHOW_INLINE', target: 'agent', requiresConfirmation: false },
  ...
  { name: '/tool install', description: 'Install a tool from URL', usage: '/tool install [url]', action: 'SHOW_INLINE', target: 'tool-install', requiresConfirmation: true },
  { name: '/tool list', description: 'List installed tools', usage: '/tool list', action: 'SHOW_INLINE', target: 'tool-list' },
  { name: '/tool health', description: 'Check tool health', usage: '/tool health', action: 'SHOW_INLINE', target: 'tool-health' },
  { name: '/tool health-all', description: 'Check all tools health', usage: '/tool health-all', action: 'SHOW_INLINE', target: 'tool-health-all' },
  { name: '/tool diagnose', description: 'Run tool diagnostic', usage: '/tool diagnose', action: 'SHOW_INLINE', target: 'tool-diagnose', requiresConfirmation: true },
...
export function getTabCompletion(input: string): string[] {
  ...
  return COMMAND_REGISTRY.filter(c => c.name.startsWith(partial)).map(c => c.name)
}
...
export function executeCommand(input: string): CommandResult {
  ...
  const commandDef = COMMAND_REGISTRY.find(c => c.name === parsed.command)
...
export function formatHelp(): string {
  return 'Comandi disponibili:\n' + COMMAND_REGISTRY.map(c => `${c.name} — ${c.description}`).join('\n')
}
